...
instance MonadHTTP m => MonadHTTP (HttpT m) where
  httpBS = MT.lift . httpBS
...
newtype HttpT m a = HttpT (MR.ReaderT Configuration m a)
  deriving (Functor, Applicative, Monad, MR.MonadReader Configuration)

instance MT.MonadTrans HttpT where
  lift = HttpT . MT.lift

instance MIO.MonadIO m => MIO.MonadIO (HttpT m) where
  liftIO = HttpT . MIO.liftIO

-- | Utility type which uses 'IO' as underlying monad
type HttpM a = HttpT IO a

-- | Run a 'HttpT' monad transformer in another monad with a specified configuration
runWithConfiguration :: Configuration -> HttpT m a -> m a
runWithConfiguration c (HttpT r) = MR.runReaderT r c
...
doCallWithConfigurationM ::
  ...
  Text ->
  [QueryParameter] ->
  HttpT m (HS.Response B8.ByteString)
